	...
	{ // process with parametres
		...
		case 4:
			...
			if (strcmp(argv[1], "--vadd") == 0) // processing 1nd parametr
				...
				init.error = E_WRONG_PARAM;
			if (init.error == E_OK)
			{ // saving filenames to structure if is everytihing OK so far...
				init.fileName[0] = argv[2];
...
int checkFiles(TParams *p_params, TSettings *p_settings, int argc)
{ //tries to open file (reading mode only)
	int ok = 1, counter = 0;

	if ((p_settings->file[counter] = fopen(p_params->fileName[counter], "r")) == NULL)
		ok = 0;
	if (argc != 3)
	{			   // case where 2 files are set in parametres -. 2 files have to be open
...
int readHeadFromFile(TParams *p_params, TSettings *p_settings)
{
	...
	for (MCounter = 0; MCounter < (p_params->matrix_num - 1); MCounter++)
	{ // '-1' because a result matrix is defined elsewhere
		...
		if (times > 0 && times < 4)
		{
			...
			if (matrix->type == 3)
			{										   // if case of 'vector of matrix' - overwrite previous setting
...
int allocMemory(TParams *p_params, TSettings *p_settings)
{
	...
	else if (p_settings->matrix[counter]->type != 3)
	{	// state == S_TEST - 1 matrixes
...
int controlProcedure(TParams *p_params, TSettings *p_settings, int argc)
{ // tries to open file (reading mode only) and alloc enough memory
	int ok = 1, counter = 0;
